...
public record PreparedShaderPack(String filename, List<Pass> passes) {
    ...
    public record Pass(String name, String vertex, String fragment) {}
    ...
    public static PreparedShaderPack read(Path path) throws IOException {
        try (var archive = new ShaderPackArchive(path)) {
            ...
            for (int index = 0; index <= 100; index++) {
                ...
                if (Pattern.compile("\\bcolortex[0-9]+(?:Format|Clear|ClearColor|MipmapEnabled)\\b|\\b(?:superSamplingLevel|noiseTextureResolution)\\b")
                    .matcher(fragmentSource).find()) throw new IOException("This pack requires unsupported buffer configuration in " + name);
                var outputs = Pattern.compile("(?:DRAWBUFFERS|RENDERTARGETS)\\s*:([^*]*)\\*/").matcher(fragmentSource);
                while (outputs.find()) if (!outputs.group(1).trim().equals("0"))
                    throw new IOException("This pack requires multiple color outputs in " + name);
                passes.add(new Pass(name, vertex ? ShaderSource.translate(archive.expand(name + ".vsh").source(), true)
                    : ShaderSource.DEFAULT_VERTEX, ShaderSource.translate(fragmentSource, false)));
            }
