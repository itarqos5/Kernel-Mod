...
public final class ShapeGridBenchmark {
    ...
    public static void run() throws Throwable {
        ...
        for (int size : new int[]{1, 8, 16}) for (String mode : new String[]{"disjoint", "last", "first", "unaligned"}) {
            if (size == 1 && mode.equals("unaligned")) continue;
    ...
    private static Query[] queries(int size, String mode) throws Exception {
        ...
        for (int sample = 0; sample < result.length; sample++) {
            int firstX = mode.equals("unaligned") ? size / 2 : size;
            ...
            if (mode.equals("first")) { first.fill(0, 0, 0); second.fill(0, 0, 0); }
            if (mode.equals("last")) { first.fill(firstX - 1, size - 1, size - 1); second.fill(size - 1, size - 1, size - 1); }
            result[sample] = new Query(ShapeGridSmokeChecks.cube(firstX, size), ShapeGridSmokeChecks.cube(size, size),
