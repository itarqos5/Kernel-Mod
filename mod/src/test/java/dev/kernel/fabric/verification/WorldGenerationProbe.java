...
final class WorldGenerationProbe {
    ...
    private static final boolean END = Boolean.getBoolean("kernel.worldProbe.end");
        ...
        try {
            ...
            if (stage == 1) {
                ...
                if (GuiProbe.screen(minecraft) instanceof CreateWorldScreen create) {
                    ...
                    state.setName(END ? "Kernel End Generation Probe" : "Kernel World Generation Probe"); state.setSeed("2718281828");
            ...
            if (stage == 2 && minecraft.level != null && minecraft.player != null && GuiProbe.screen(minecraft) == null) {
                ...
                server.execute(() -> {
                    try {
                        ...
                        var level = END ? server.getLevel(Level.END) : server.overworld();
                        if (level == null) throw new AssertionError("Missing requested dimension");
                        int baseX = END ? 128 : 1024, baseZ = END ? 176 : 1024;
                        ...
                        for (var status : new ChunkStatus[]{ChunkStatus.CARVERS, ChunkStatus.FEATURES}) {
                            for (int x = baseX - 1; x < baseX + 4; x++) for (int z = baseZ - 1; z < baseZ + 4; z++) {
                                level.getChunkSource().getChunk(x, z, status, true);
                            }
                        }
                        for (int x = baseX; x < baseX + 3; x++) for (int z = baseZ; z < baseZ + 3; z++) level.getChunk(x, z);
                        var digest = MessageDigest.getInstance("SHA-256");
                        long nonAir = 0;
                        try (var output = new DataOutputStream(new DigestOutputStream(OutputStream.nullOutputStream(), digest))) {
                            for (int x = baseX; x < baseX + 3; x++) for (int z = baseZ; z < baseZ + 3; z++) {
                                ...
                                for (var section : chunk.getSections()) {
                                    for (int sy = 0; sy < 16; sy++) for (int sx = 0; sx < 16; sx++) for (int sz = 0; sz < 16; sz++) {
                                        ...
                                        output.writeInt(Block.getId(state));
                        ...
                        if (END && nonAir < 4096) throw new AssertionError("End fixture contains too little terrain: " + nonAir);
                        Files.writeString(minecraft.gameDirectory.toPath().resolve("world-generation-sha256.txt"), hash + "\n");
                        System.out.println("Kernel world probe: " + (END ? "End" : "Overworld") + ", 9 remote chunks, blocks and biomes SHA-256=" + hash
                            + "; non-air blocks=" + nonAir + "; generation+fingerprint ms=" + (System.nanoTime() - start) / 1_000_000L);
                        completed = hash;
