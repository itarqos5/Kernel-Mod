...
- A lightning-icon button left of Options in title/pause menus opens Kernel video settings, also available through the native video settings list. Four video/optimization tabs plus Shaders use translucent panels and white highlights. The screen is a fixed frame around one scrolling, section-grouped option list with a description panel beneath it, in the manner of Sodium and VulkanMod; rows scroll by whole entries so none is drawn clipped at any GUI scale. Video controls preserve native callbacks, Apply/Done/Cancel drafts and restart-only optimization switches. English fallbacks and the icon work without Fabric API. See [settings and recovery](docs/RENDERER_SETTINGS.md).
...
- Full-world shader-pack support, including terrain/geometry replacement, shadows, `prepare` stages, separate opaque-depth stages, hand projection inputs, integer/other unsupported color formats, non-PNG/resource-pack textures and the shader properties that change how a pass draws. Kernel now reads the Iris pack format's options, screens and dimension folders, but a pack that ships `gbuffers_*`, `shadow*` or `prepare*` programs is still refused by name rather than rendered incorrectly. Popular full-world packs are not compatible with the limited color post-processing renderer yet.
