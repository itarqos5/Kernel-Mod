...
- An original OpenGL color post-processing shader pipeline with ordered composite/final passes, native Shaders GUI, Modrinth discovery/install, ZIP drag/drop, verified downloads, persistent selection and failed-pack recovery. Conditional and guarded literal includes use the native GLSL preprocessor, with bounded expansion and source diagnostics. Up to sixteen color buffers and eight simultaneous outputs support explicit target routing and indexed graphics-state restoration. Twelve normalized/floating-point formats, per-pass color mipmaps, literal clear settings and retained auxiliary history support feedback across passes and frames, with reset on resizing or world changes. Unsupported terrain/shadow/depth stages and integer formats remain explicitly rejected. See [shader support and limits](docs/SHADERS.md).
...
- Full-world shader-pack support, including terrain/geometry replacement, shadows, depth effects, integer/other unsupported color formats, custom textures and shader properties/options. Popular full-world packs are not compatible with the limited color post-processing renderer yet.
