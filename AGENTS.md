...
- Immutable launch-time renderer configuration in `config/kernel-renderer.properties`, with eleven per-feature controls, complete dependent-Mixin group activation, version-aware availability, preserved unknown keys, atomic save where supported and fail-open disabling on malformed/unreadable settings.
