...
- [Original color post-processing](SHADERS.md) with sixteen logical color buffers and eight simultaneous outputs, Modrinth discovery/installation, ZIP drag/drop and Iris
  incompatibility are implemented. Full-world shader rendering still requires terrain/shadow/depth,
  custom buffer formats, texture and shader-properties support; a downloader or partial pipeline is not full compatibility.
